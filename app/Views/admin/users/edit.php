...
<?= $this->section('content') ?>
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="mb-1 text-light">Edit User</h1>
            <div class="text-secondary">Update user information</div>
        </div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="password" class="form-control <?= ($validation && $validation->hasError('password_confirm')) ? 'is-invalid' : '' ?>" 
                               id="password_confirm" name="password_confirm" placeholder="Confirm new password">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select <?= ($validation && $validation->hasError('role')) ? 'is-invalid' : '' ?>" 
                            ...
                            <option value="student" <?= (old('role') ?? $user['role_display'] ?? '') === 'student' ? 'selected' : '' ?>>Student</option>
                            <option value="instructor" <?= (old('role') ?? $user['role_display'] ?? '') === 'teacher' ? 'selected' : '' ?>>Teacher/Instructor</option>
                        </select>
                        <?php if ($validation && $validation->hasError('role')): ?>
                            <div class="invalid-feedback"><?= esc($validation->getError('role')) ?></div>
                        <?php endif; ?>
                        <small class="form-text text-muted">
                            <i class="bi bi-info-circle me-1"></i>Admin role is protected and cannot be assigned through this interface.
                        </small>
                    </div>
                </div>
